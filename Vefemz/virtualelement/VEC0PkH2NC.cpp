...
void VEC0PkH2NC::GetB_H2(int ElemID,double **B)
{
            ...
            {                
				...
				if(dof.Num_PerEdge>0)
                    if(j>=nv&&j<nv+nv*dof.Num_PerEdge)
                    {
						...
						int ek=edoftype%(dof.Num_PerEdge/2);//present edge dof
...
void VEC0PkH2NC::GetD(int ElemID,double **D)
{
	int i,j,k,m,n; 
...
void VEC0PkH2NC::GetG_H2(int ElemID,double **G)
{
...
void VEC0PkH2NC::GetPistar_H2(int ElemID,double **Pistar)
{
	double **B,**G,*X,*R;
...
void VEC0PkH2NC::GetPi_H2(int ElemID,double **Pi)
{
...
void VEC0PkH2NC::GetA_H2(int ElemID,double **A)
{
	int i,j,k,EdofNum=dof.Total_Num_PerElement[ElemID];
...
void VEC0PkH2NC::GetB_H1(int ElemID,double **B)
{
	int i,j,k,m,nv=ms.ElementVertex_Num[ElemID],EdofNum=dof.Total_Num_PerElement[ElemID];    
    double e1x[2],e1y[2],n1[2],x[3],y[3];
    double d1,vx,vy,vxx,vyy,temp;
    double **PistarL2toEdge=new double*[p+1],**PistarH2=new double*[polydim],**GL2=new double*[polydim];
	for(i=0;i<p+1;i++)	PistarL2toEdge[i]=new double[EdofNum];
	for(i=0;i<polydim;i++) 
	{
		GL2[i]=new double[polydim];	PistarH2[i]=new double[EdofNum];
	}
	GetG_L2(ElemID,GL2);GetPistar_H2(ElemID,PistarH2);
    double *xpos=new double[nv]; double *ypos=new double[nv];//存储单元位置   
    double hE=ms.ElementDiameter[ElemID],xE=ms.ElementBarycenter[ElemID][0],yE=ms.ElementBarycenter[ElemID][1];
    for(i=0;i<ms.ElementVertex_Num[ElemID];i++)
    {
   ...
   for(i=0;i<polydim;i++)
    ...
    for(i=0;i<polydim;i++)
    {
		...
		{
			for(int en=0;en<nv;en++) //\int_ev dp/dn ds
			{                                
				L2ProjecttoEdgePoly(ElemID,en,PistarL2toEdge);
                e1x[0]=xpos[en];    e1x[1]=xpos[(en+1)%nv];
                e1y[0]=ypos[en];    e1y[1]=ypos[(en+1)%nv];
                d1=sqrt((e1x[1]-e1x[0])*(e1x[1]-e1x[0])+(e1y[1]-e1y[0])*(e1y[1]-e1y[0]));                   
                n1[0]=(e1y[1]-e1y[0])/d1;    n1[1]=(e1x[0]-e1x[1])/d1;                        
				...
				for(k=0;k<GLQ.QuadPtsNum;k++)
				{
					double stdx=GLQ.QuadPts[k]; //积分点
					double jx,jy;				
					jx=(e1x[0]+e1x[1])/2.+(e1x[1]-e1x[0])*stdx/2.; jy=(e1y[0]+e1y[1])/2.+(e1y[1]-e1y[0])*stdx/2.;
					vx=SMS.GetDerivative(i,1,0,jx,jy,hE,xE,yE);
					vy=SMS.GetDerivative(i,0,1,jx,jy,hE,xE,yE);	
					for(j=0;j<EdofNum;j++)
					{
						temp=0;
						for(m=0;m<p+1;m++)	temp+=PistarL2toEdge[m][j]*pow(stdx,m);//v				
						B[i][j]+=(vx*n1[0]+vy*n1[1])*temp*d1/2*GLQ.Weights[k];//\int_e v dp/dn ds			
					}                                              
				}
			}
	
			if(i<EdofNum-nv-nv*dof.Num_PerEdge)
				for(j=nv+nv*dof.Num_PerEdge;j<EdofNum;j++) //int_K phi*(-Delta p_k-2)dx
				{
					int jp=j-nv-nv*dof.Num_PerEdge;
					int j_row=0,j_col=0,k;
					for(k=0;k<p-1;k++)
			...
			else //int_K Pi^*phi *(-Delta p_(k,k-1))dx
			{
				for(j=0;j<nv;j++)
				{
					x[0]=xE;x[1]=xpos[j];x[2]=xpos[(j+1)%nv];
					...
					for(int k=0;k<TQ.QuadPtsNum;k++)
					{
						...
						for(m=0;m<EdofNum;m++)
							for(int n=0;n<polydim;n++)
								B[i][m]-=(vxx+vyy)*PistarH2[n][m]*SMS.GetValue(n, jx, jy, hE, xE, yE)*vol_K*jw;
					}
				}				
			}
		}
    }
	for(i=0;i<p+1;i++)	delete[] PistarL2toEdge[i];
	for(i=0;i<polydim;i++)
	{
		delete[] GL2[i];delete[] PistarH2[i];
	}
	delete [] xpos; delete [] ypos; delete[] PistarL2toEdge;delete[] GL2;delete[] PistarH2;
}

void VEC0PkH2NC::GetG_H1(int ElemID,double **G)
{
...
void VEC0PkH2NC::GetPistar_H1(int ElemID,double **Pistar)
{
	double **B,**G,*X,*R;
	int i,j,EdofNum=dof.Total_Num_PerElement[ElemID];
	X=new double[polydim]; R=new double[polydim];
...
void VEC0PkH2NC::GetPi_H1(int ElemID,double **Pi)
{
	double **D;	double **Pistar;
	int i,j,k,EdofNum=dof.Total_Num_PerElement[ElemID];
	D=new double*[EdofNum];	Pistar=new double*[polydim];
...
void VEC0PkH2NC::GetA_H1(int ElemID,double **A)
{
	int i,j,k,EdofNum=dof.Total_Num_PerElement[ElemID];
	double **B=new double*[polydim];	double **G=new double*[polydim];
	double **GB=new double*[polydim];
...
void VEC0PkH2NC::L2ProjecttoEdgePoly(int ElemID,int EdgeID,double **Pistar)
{
	int i,j,k,en,NdofNum=dof.Total_Num_PerElement[ElemID],nv=ms.ElementVertex_Num[ElemID];
	int eflag=ms.ElementEdgeFlag[ElemID][EdgeID];
	double x[2],y[2],stdx;
	en=ms.Element[ElemID][EdgeID];
	x[0]=ms.Node[en][0]; y[0]=ms.Node[en][1]; 
	en=ms.Element[ElemID][(EdgeID+1)%nv];
	x[1]=ms.Node[ en ][0]; y[1]=ms.Node[ en ][1]; 
	double **G=new double *[p+1];	double **B=new double *[p+1];
	for(i=0;i<p+1;i++)
	{
		G[i]=new double[p+1];	B[i]=new double[NdofNum];
	}
	...
	for(i=0;i<p+1;i++)
	{
		if(i==0)
			...
			for(j=0;j<p+1;j++)
			{
				...
				{
					stdx=glq.QuadPts[k];
					G[i][j]+=pow(stdx,i-2)*pow(stdx,j)/2.*glq.Weights[k];
				}
			}
	}
	...
	for(i=0;i<p+1;i++)
		for(j=0;j<NdofNum;j++)
			B[i][j]=0;			
	B[0][EdgeID]=1;B[1][(EdgeID+1)%nv]=1;					
	for(i=2;i<p+1;i++)		
		for(j=nv;j<nv+nv*dof.Num_PerEdge;j++)
		{	
			int en1=(j-nv)/dof.Num_PerEdge;//present edge     
			int edoftype=(j-nv)%dof.Num_PerEdge;
			int ek=edoftype%(dof.Num_PerEdge/2);//present edge dof
			if(en1==EdgeID&&edoftype<dof.Num_PerEdge/2&&ek==i-2)
				B[i][j]=pow(eflag,i-2);;
		}
	double *R=new double[p+1];	double *X=new double[p+1];
	for(i=0;i<NdofNum;i++)
	{
		for(j=0;j<p+1;j++)
			R[j]=B[j][i];
		GaussSolve(p+1,G,R,X);
		for(j=0;j<p+1;j++)			
			Pistar[j][i]=X[j];
	}
	for(i=0;i<p+1;i++)
	{
		delete [] G[i];	delete [] B[i];
	}
...
void VEC0PkH2NC::GetB_L2(int ElemID,double **B)
{
...
void VEC0PkH2NC::GetG_L2(int ElemID,double **G)
{
	int i,j,nv=ms.ElementVertex_Num[ElemID];
    for(i=0;i<polydim;i++)
        for(j=0;j<polydim;j++)
...
void VEC0PkH2NC::GetPistar_L2(int ElemID,double ** Pistar)//Pistar=G^-1*B
{
    double **B,**G,*X,*R;
    int i,j,EdofNum=dof.Total_Num_PerElement[ElemID];
    X=new double[polydim]; R=new double[polydim];
...
void VEC0PkH2NC::GetPi_L2(int ElemID,double **Pi) //Pi=D*Pistar
{
    double **D,**Pistar;
    int i,j,k,EdofNum=dof.Total_Num_PerElement[ElemID];
    D=new double*[EdofNum];    Pistar=new double*[polydim];
...
void VEC0PkH2NC::GetA_L2(int ElemID,double **A)
{
    int i,j,k,EdofNum=dof.Total_Num_PerElement[ElemID];
    double hE=ms.ElementDiameter[ElemID];
...
void VEC0PkH2NC::GetRHSL2B(int ElemID,FunctionP Source,double *LocFB)
{
...
void VEC0PkH2NC::GetRHS(int ElemID,FunctionP Source,double *LocF)
{
...
void VEC0PkH2NC::GetBdof_BdofVal(FunctionP BFunc,int *Bdof,double *Bdofval)
{
	int i,j,k,m,n=0,en;
...
void VEC0PkH2NC::GetDofVal(FunctionP u,double *uI)
{
	int i,j,k,m,en,nv;
...
void VEC0PkH2NC::GetRHSL2B(int ElemID,FunctionPt Source,double t,double *LocFB)
{
    int i,j,k,nv=ms.ElementVertex_Num[ElemID],polydim1;
    if(p==2||p==3)    polydim1=p*(p-1)/2;
    else if(p==4)    polydim1=3;
...
void VEC0PkH2NC::GetRHS(int ElemID,FunctionPt Source,double t,double *LocF)
{
    int i,j,m,n,EdofNum=dof.Total_Num_PerElement[ElemID],polydim1,nv=ms.ElementVertex_Num[ElemID];
    if(p==2||p==3)    polydim1=p*(p-1)/2;
    else if(p==4)    polydim1=3;
    ...
    GetRHSL2B(ElemID,Source,t,LocFB);    GetG_L2(ElemID,G);
    GaussSolve(polydim1,G,LocFB,XX);
...
void VEC0PkH2NC::GetBdof_BdofVal(FunctionPt BFunc,double t,int *Bdof,double *Bdofval)
{
    int i,j,k,m,n=0,en;
    double d,temp,value[2],x,y,ex[2],ey[2];
...
void VEC0PkH2NC::GetDofVal(FunctionPt u,double t,double *uI)
{
    int i,j,k,m,en,nv;
    double px,py,temp,d,ex[2],ey[2],n1[2];
    double value[3];
